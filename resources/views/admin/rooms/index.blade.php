                ...
                @foreach ($rooms as $room)
                    <tr class="divide-x divide-gray-200">
                        ...
                        <td class="flex justify-center px-6 py-2">
                            <button onclick="listRoom('{{ $room->id }}')" class="p-2 rounded bg-yellow-500
                                hover:bg-yellow-800 text-white transition-colors duration-300">
                                <i class="far fa-eye"></i>
                            </button>
                            <a href="{{route('roomEdit', ['id' => $room->id])}}" class="p-2 ml-2 rounded
                                bg-blue-500 hover:bg-blue-800 text-white transition-colors duration-300">
                                <i class="fas fa-pencil-alt"></i>
                            </a>
                            <form class="ml-2" action="{{ route('roomDestroy', ['id' => $room->id]) }}"
