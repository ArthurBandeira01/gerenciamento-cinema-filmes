...
<div class="main-content mt-5">
    ...
    @if(isset($tenantVerify)) action="{{ route('usersTenantUpdate', ['user' => $user->id]) }}">
    @else action="{{ route('usersUpdate', ['user' => $user->id]) }}">
    @endif
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block text-gray-700 text-sm font-bold mb-2 text-left">Nome</label>
            <input type="text" id="name" value="{{$user->name}}" name="name" placeholder="Seu nome"
        ...
        <div class="mb-4">
            ...
            <input type="email" id="email" value="{{$user->email}}" name="email" placeholder="Seu e-mail"
            class="validate border rounded-lg px-3 py-2 w-full focus:outline-none focus:border-blue-500">
        </div>
        <div class="mb-6">
            <label for="password" class="block text-gray-700 text-sm font-bold mb-2 text-left">Senha</label>
            <input type="password" id="password" value="" name="password" placeholder="Sua senha"
            class="validate border rounded-lg px-3 py-2 w-full focus:outline-none focus:border-blue-500">
        </div>
        <div class="flex items-center justify-center">
            <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-2
